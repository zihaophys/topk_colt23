...
function stop(criterion::LLR_Stopping, pep, β, t, δ, θ_hat, Vinv, N)
    val, _ = min_alt(pep,θ_hat, Vinv, N)
    if val > β(t,δ)
...
function stop(criterion::Force_Stopping, pep, β, t, δ, θ_hat, Vinv, N)
    if t >= criterion.max_samples
        return true, istar(pep,θ_hat)
    end
    return stop(criterion.base_stopping, pep, β, t, δ, θ_hat, Vinv, N)
end
