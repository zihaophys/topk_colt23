...
"""
Lazy Mirror Ascent (Pierre Ménard 2019)
"""
...
long(sr::Menard) = "LMA" * abbrev(sr.ElimType);
abbrev(sr::Menard) = "LMA" * abbrev(sr.ElimType);

function start(sr::Menard, pep, N)
    MenardState(sr.TrackingRule, sr.scale, N, sr.ElimType);
end
...
struct MenardState
    ...
    t;
    ElimType;
    MenardState(TrackingRule, scale, N, ElimType) = new(
...
function nextsample(sr::MenardState, pep::Union{BAI_State, Topm_State, OSI_State}, β, t, δ, θ_hat, N, S, Vinv, R)
    ...
    ∇ = zeros(K)
    for i in topm_active  # loop over (active) topm arms
        candidates = setdiff(1:K, topm)
        for j in candidates # loop over (active) "ambiguous arms"
            tmp = zeros(K)
            dij = (arms[i]'θ_hat * w[i] + arms[j]'θ_hat * w[j]) / (w[i] + w[j])
            tmp[i] = dKL(arms[i]'θ_hat, dij)
            tmp[j] = dKL(arms[j]'θ_hat, dij)
            cij = w[i] * tmp[i] + w[j] * tmp[j]
            if cij < minC
                minC = cij
                ∇ = tmp
            end
    ...
    incur!(sr.h, -∇);
    return track(sr.t, N, w), false, Nothing
end
