...
function play!(k, rng, pep, θ, S, N, Vinv, R)
    ...
    Y = (rand() < ϕ'θ);  # sample reward from N(ϕ'θ, 1)
...
function runit(seed, sampling, stopping, elim_rule::Union{NoElim,CompElim,StatElim}, θ, pep::Union{BAI,Topm,OSI}, β, δ, reset_active=false)
    ...
    while true
        ...
        # check stopping rule
        should_stop, answer = stop(stopping, pep, β, t, δ, θ_hat, Vinv, N)
        if should_stop
            # println("End ", alg_name, " -- seed ", seed, " -- samples ", sum(N), " -- time ", (time_ns() - start_time)/1e9, " s\n")
            return (reshape(N, (1, K)), is_correct(pep, θ, answer))
        end
        ...
        if internal_stop
            # println("End ", alg_name, " -- seed ", seed, " -- samples ", sum(N), " -- time ", (time_ns() - start_time)/1e9, " s\n")
            return (reshape(N, (1, K)), is_correct(pep, θ, internal_answer))
        end
