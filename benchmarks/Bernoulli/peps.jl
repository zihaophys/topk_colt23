...
# closest alternative and value for all pieces
function min_alt(pep::Union{Topm, Topm_State}, θ, Vinv, N)
    topm = istar(pep, θ)
    return minimum(alternative(Vinv, θ, pep.arms[j], pep.arms[k], N[j], N[k]) for j in topm, k in setdiff(1:narms(pep), topm))
end
